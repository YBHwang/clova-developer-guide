...
| `client_id`     | ID used by Clova to get the access token for the third-party service. You must register a `cliend_id` in advance on the Clova Developer Center.     |
| `response_type` | Parameter that has a defined OAuth 2.0 authorization type. Use the `"code"` type. You can specify the type on the Clova Developer Center. Currently, only `"code"` type is supported.|
| `scope`         | `scope` field of OAuth. You can use to define the access level. You must register the `scope` in advance on the Clova Developer Center.                            |
| `redirect_uri`  |URL used to redirect the user after authentication (redirect URL). The `redirect_uri` value can be verified when [setting up account linking](/DevConsole/Guides/CEK/Register_Extension.md#SetAccountLinking) to register the extension on the Clova Developer Center. Currently, `{{ book.RedirectURLforAccountLinking }}` is used. |
...
<div class="note">
<p><strong>Note!</strong></p>
<p>The <code>redirect_uri</code> can be found on the <a href="/DevConsole/Guides/CEK/Register_Extension.md#RedirectURI">Setting up account linking</a> page of the Clova Developer Center.</p>
</div>
...
| Parameter name  | Description                                                                                                                                                      |
|:---------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------|
| `vendorId`     | ID assigned to the extension developer: This ID is registered in the Clova Developer Center in order to distinguish a company or a third-party service. It is included in `redirect_uri` in advance. |
...
After building an authorization server and applying account linking to the extension, you need to register the information mentioned in [Building an authorization server](#BuildAuthServer) on the [Clova Developer Center](/DevConsole/ClovaDevConsole_Overview.md). In the extension registered on the Clova Developer Center, [enter the account linking information](/DevConsole/Guides/CEK/Register_Extension.md#SetAccountLinking) as follows:
