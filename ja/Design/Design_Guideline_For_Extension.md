...
* [呼び出し名を定義する](#DefineInvocationName)
...
### 呼び出し名の要件 {#InvocationNameRequirement}

呼び出し名は、次の要件を満たしていることが必要です。
1. [利用規約に違反していないこと](#TermsOfUse)
2. [単語一語または汎用的な言葉でないこと](#WordsInGeneral)
3. [人名・地名でないこと](#PersonsName)
4. [Clovaで使用されている主要なフレーズや、つなぎ語を含まないこと](#PhrasesForClova)
5. [他スキルと同一または類似する名称でないこと](#OtherSlills)
6. [日本のユーザーを対象とした言語であること](#TargetLanguage)
7. [誤解を招く表現が含まれないこと](#AvoidMisunderstanding)
8. [その他の留意事項](#OtherNotices)

#### 1. 利用規約に違反していないこと {#TermsOfUse}

作成するExtensionは、第三者の権利の侵害など、[Clova Extensions Kit利用規約](https://clova-developers.line.me/cek/#/terms)に違反するものであってはなりません。特に、スキル名／呼び出し名については、利用規約第6条に記載された各号に定める開発スキルに関する事項を遵守しなければなりません。

  <details><summary>Clova Extensions Kit利用規約 第6条</summary>
  (1) ガイドライン等の内容を充足し、ガイドライン等に定める禁止事項に該当しないこと<br>
  (2) サーバーの保守が必要であるなどやむを得ない場合を除き、エンドユーザーのリクエストに対して常に正常に応答できる内容であること<br>
  (3) エンドユーザーに対して開発者その他第三者の商品、サービス等の広告を行うものではないこと（但し、当社の事前の承諾がある場合を除きます。）<br>
  (4) 第三者の権利（著作権、著作者人格権、特許権、商標権、意匠権、実用新案権、営業秘密、名誉権、肖像権、プライバシー権、パブリシティー権を含みますが、これらに限りません。）を侵害していないこと<br>
  (5) 当社又は第三者を不当に差別若しくは誹謗中傷し、又はそれらを助長するものでないこと<br>
  (6) Clova利用データ等その他の個人情報の収集を目的としたものでないこと<br>
  (7) コンピュータウィルス等の有害なプログラムを含むものでなく、かかるプログラムを送信し、又は送信する機能を含むものでないこと<br>
  (8) 当社又は第三者の機器、設備、システム等の利用若しくは運用を妨げ、又はかかる利用若しくは運用に支障を与えないこと<br>
  (9) 当社又は第三者の設備に蓄積された情報を不正に書き換え、又は消去する機能を有しないこと<br>
  (10) 当社が開発スキルを開発若しくは提供し、又は当社と開発者が提携していると誤解を与え、又はそのおそれのあるものでないこと<br>
  (11) 賭博その他の違法なギャンブルを行わせ、又は賭博その他の違法なギャンブルへの参加を勧誘するものでないこ<br>
  (12) 人の生命又は身体の安全を害するおそれのある行為を助長し、又は促進するものでないこと<br>
  (13) グロテスク、露骨な性描写、児童ポルノ若しくは児童虐待にかかる表現、その他残虐若しくはわいせつな表現、又はこれらを連想させる表現をするものでないこと<br>
  (14) 面識のない異性との出会い等を目的とする内容でないこと<br>
  (15) 青少年の家出を誘引・助長する内容でないこと<br>
  (16) その他、犯罪を構成若しくは助長し、又は公序良俗若しくは法令等に違反する、又はそれらのおそれのあるものでないこと<br>
  </details>

#### 2. 単語一語または汎用的な言葉でないこと {#WordsInGeneral}

単語一語の呼び出し名は、許可されません。また、「スポーツニュース」等の汎用的な言葉は許可されませんので、開発者名やサービス名等をかけ合わせることを推奨します（例：「太郎のスポーツニュース」）。  
例外として、独自のブランドまたはサービス名等は使用することができます。

#### 3. 人名・地名でないこと {#PersonsName}

人名や地名のみで構成されたスキル名／呼び出し名は許可されません。
...
Clovaで使用されている主要なフレーズ等を含めることはできません。
* 呼び名
  * 例：「ねぇClova」「Clova」
* 起動フレーズ
  * 例：「を起動して」
...
なお、呼び出し名は、呼び名や起動フレーズと組み合わせたときに、自然な言い回しである必要があります。

#### 5. 他スキルと同一または類似する名称でないこと {#OtherSlills}

ビルトインを含めた他のスキルで利用されている呼び出し名は使用しないでください。また、他スキルと類似している場合や、システムに悪影響する場合も許可されない可能性があります。  

#### 6. 日本のユーザーを対象とした言語であること {#TargetLanguage}

原則として、日本のユーザーを対象としたExtensionのスキル名と呼び出し名には、日本語を使用してください。  
例外として、独自のブランド名／知的財産、名前や場所等の固有名詞等は使用可能です。

#### 7. 誤解を招く表現が含まれないこと {#AvoidMisunderstanding}

以下に該当する場合は、呼び出し名が許可されません。
* 実在しない法人・団体・個人事業主または商品、サービスがあるかのように誤解される可能性がある。
* スキル名や呼び出し名と、コンテンツの内容がそれぞれ異なる、または誤解を招く可能性がある。
* 呼び出し名（メイン）と呼び出し名（サブ）の内容がそれぞれ異なるまたは誤解を招く可能性がある。
* LINEまたは関連会社のサービスと誤解される可能性がある。
...
<div class="note">
  <p><strong>メモ</strong></p>
  <p>一部例外がある場合がありますので、判断が難しい場合はお問い合わせください。</p>
</div>
