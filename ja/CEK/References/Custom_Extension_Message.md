...
| `event.name`      | string  | {{ "クライアントがClovaに送信したメッセージの名前" if book.TargetCountryCode == "KR" else "クライアントがClovaに送信したイベントの名前、または、スキルが有効か無効かを示す名前。スキルが有効か無効かを示す名前は、`SkillEnabled`か`SkillDisabled`になります。スキルが有効か無効かを示すリクエストを受けると、[リクエストメッセージ](#CustomExtRequestMessage)の`context.System.application.applicationId`フィールドと`context.System.user.userId`フィールドを利用してユーザー情報を収集または廃棄するように実装できます。" }} |    |
