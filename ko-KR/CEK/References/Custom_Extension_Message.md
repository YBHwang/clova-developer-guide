...
| `timestamp`       | string  | 클라이언트가 Clova로 정보를 전달한 시간(`YYYY-MM-DDThh:mm:ssZ`<a href="https://en.wikipedia.org/wiki/ISO_8601#Combined_date_and_time_representations" target="_blank">ISO 8601</a> 형식)<div class="note"><p><strong>Note!</strong></p><p>CEK는 <code>EventRequest</code> 타입 요청 간의 순서를 보장하지 않기 때문에 이 필드 값을 활용하여 클라이언트의 요청의 순서를 파악할 수 있습니다.</p></div>                    |   |
