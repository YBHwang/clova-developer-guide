...
2. 클라이언트는 CIC로부터 지시 메시지를 받으면, [지시 메시지의 헤더](/CIC/References/CIC_API.md#Directive)에 포함된 대화 ID와 클라이언트가 저장해 둔 마지막 대화 ID를 비교해야 합니다.
  ...
  * 클라이언트는 새 대화 ID를 가진 지시 메시지를 처리하기 위해 다음과 같은 것을 수행해야 합니다.
    * 이전 대화 ID를 가진 지시 메시지의 내용을 사용자에게 제공하고 있다면 이를 즉시 중단해야 할 수 있습니다.
    * [기본 오디오 재생 규칙](/Design/Design_Guideline_For_Client_Hardware.md#AudioInterruptionRule)이나 [사용자 발화 시 오디오 재생 규칙](/Design/Design_Guideline_For_Client_Hardware.md#AudioInterruptionRuleForUserUtterance)을 참고하여 메시지 큐에서 이전 대화 ID를 가진 지시 메시지를 모두 폐기해야 합니다. 이를 위해 메시지 큐에 있는 지시 메시지의 대화 ID를 항상 확인해야 합니다.
