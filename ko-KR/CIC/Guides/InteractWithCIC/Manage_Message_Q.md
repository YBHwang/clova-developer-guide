...
* 수신된 지시 메시지의 [대화 ID](/CIC/Guides/Implement_Client_Features.md#ManageDialogueIDAndHandleTasks)는 서로 다를 수 있습니다.
...
개발자는 각 메시지 큐에 쌓인 지시 메시지가 하나씩 처리되도록 개발해야 하며, 이때 클라이언트가 가지고 있는 사용자의 마지막 요청에 대응하는 메시지가 처리되도록 해야 합니다. 사용자의 요청은 [대화 ID](/CIC/Guides/Implement_Client_Features.md#ManageDialogueIDAndHandleTasks)로 식별할 수 있으며, 클라이언트는 마지막 요청에 대한 대화 ID를 보관하고 있어야 합니다. 만약 클라이언트가 이미 취소된 대화 ID를 가진 지시 메시지를 큐에 보관하고 있다면 해당 메시지와 그 메시지에 상응하는 부가 정보를 큐에서 제거해야 합니다.
